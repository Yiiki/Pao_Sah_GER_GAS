function vll=quasi_equilibrium(vgs0,vds0)
...
% recombination rate
rr=Cq.*VT.*free_electron_mass.^-1.*mu0.^-1.*...
    r(uv_mat(1,:),uv_mat(2,:));
% rr=4.*pi.*elementary_charge.^2.*Planck_constant.^-2.*VT.*mu0.^-1.*...
%     r(uv_mat(1,:),uv_mat(2,:));
% rr=R0.*elementary_charge.^-1.*V_T0.*VT.*mu0.*Cq.*L0.^-2.*...
%     r(uv_mat(1,:),uv_mat(2,:));
rr_aid=kron([1;1],rr);
% K-field
Kuv=elementary_charge.^-1.*VT.*Cox.*[kapa_e.*mu_e.*[1 1]
              kapa_h.*mu_h.*[1 1]].*uv_mat;
% vds-lower limits
vll=-0.5.*ch_l.^2.*rr_aid./Kuv;
% vll=uv_mat(1,:).*uv_mat(2,:)-ubar*vbar;
%% function definition
% approximate $\xi_n$
function y=xinq(x)
